...
def summarize(r):
    ...
    holds=[x.get('holding_minutes',(x['exit_time']-x['entry_time'])/60) for x in t]
    return {'trades':m['trades'],'win_rate':m['win_rate'],'average_winner':m['average_win'],'average_loser':m['average_loss'],'profit_factor':m['profit_factor'],'gross_expectancy':float(np.mean(gross))if gross else None,'net_expectancy':m['expectancy'],'net_pnl':m['net_pnl'],'max_drawdown':m['max_drawdown'],'mean_mae':float(np.mean([x['mae']for x in t]))if t else None,'max_mae':m['maximum_adverse_excursion'],'mean_mfe':float(np.mean([x['mfe']for x in t]))if t else None,'max_mfe':m['maximum_favorable_excursion'],'average_holding_minutes':float(np.mean(holds))if t else None,'raw_sharpe':m['sharpe'],'raw_sortino':m['sortino'],'daily_observations':m['daily_observations']}
