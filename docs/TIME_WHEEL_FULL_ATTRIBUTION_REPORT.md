...
This fully separates the exit families that were shared in the prior experiment. A–D use the same MT5 XAUUSD M1 dataset, chronological split, cost of 0.40, next-open entry, stop-first ambiguity rule, adverse stop-gap fill, one-position constraint, and 30-bar maximum hold. E is the immutable current production baseline. The [frozen protocol](experiments/TW-FULL-EXIT-1-20260910T145743432784Z/TIME_WHEEL_FULL_ATTRIBUTION_PROTOCOL.md) defines every rule before results.
...
All A–D signals, levels, rejects, trades, equity curves, and full raw metrics are in compressed artifacts next to [comparison.json](experiments/TW-FULL-EXIT-1-20260910T145743432784Z/comparison.json). Source and SQLite logical fingerprints were rechecked after the run; production defaults and stored records are unchanged. Thirteen research tests passed. Re-run with:
